...
export class MetaError extends Error {
  constructor(message, status = 502, detalhe = null) {
    super(message);
    this.name = 'MetaError';
    this.status = status;
    // code/subcode da Meta: é o que diferencia token expirado de app
    // bloqueado de permissão faltando. Sem isso a mensagem sozinha não
    // diz o que fazer.
...
async function request(url) {
  ...
  if (!res.ok || json?.error) {
    ...
    const msg = err.error_user_msg || err.message || `Graph API respondeu ${res.status}`;
    const detalhe = {
      http: res.status,
      code: err.code ?? null,
      subcode: err.error_subcode ?? null,
      type: err.type ?? null,
      titulo: err.error_user_title ?? null,
      trace: err.fbtrace_id ?? null,
    };
    throw new MetaError(msg, res.status === 401 || res.status === 403 ? 403 : 502, detalhe);
  }
  return json;
}
 ...
 */
/* O "Leads (form)" que o Gerenciador mostra não existe como um tipo de
   ação isolado. O que a Meta devolve é o total de leads e, à parte, os
   leads que não vieram do formulário — a diferença entre os dois é o
   número da tela. Conferido em duas campanhas da conta entre 24 e
   30/07: 155-23=132 e 175-6=169, batendo com o Gerenciador nas duas. */
const LEAD_TYPES = [
  'lead',
...
const LEAD_FORA_DO_FORMULARIO_TYPES = [
...
export function normalizeInsight(row) {
  const leadsTotal = firstAction(row.actions, LEAD_TYPES);
  const leadsForaDoForm = firstAction(row.actions, LEAD_FORA_DO_FORMULARIO_TYPES);
  const leads = Math.max(0, leadsTotal - leadsForaDoForm);
...
const INSIGHT_FIELDS_DIA = [
  'campaign_id', 'spend', 'impressions', 'clicks', 'actions',
].join(',');

/**
 * Série diária por campanha. É o que permite ao gráfico responder aos
 * filtros — uma série no nível de conta não poderia ser recortada.
 *
 * No nível de campanha, e não de anúncio, porque as três dimensões de
 * corte (área, tipo e destino) são atributos da campanha: a granularidade
 * por anúncio multiplicaria as linhas sem mudar nenhum recorte, e a Meta
 * limita a carga de chamadas de insights.
 */
export async function fetchDailyCampaignSeries(accountId, timeRange) {
  const rows = await graphAll(`act_${accountId}/insights`, {
    level: 'campaign',
