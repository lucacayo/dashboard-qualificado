...
/** Métricas derivadas — recalculadas a partir dos totais, nunca somadas. */
export function withDerived(base) {
  ...
  return {
    ...
    custo_por_resultado: resultados > 0 ? spend / resultados : 0,
    /* Custo por lead e por conversa dividem o MESMO investimento por
       denominadores diferentes, porque a Meta não reparte o gasto entre
       os tipos de resultado. Por isso os dois não somam o custo por
       resultado — é assim que o Gerenciador também calcula. */
    custo_por_lead: leads > 0 ? spend / leads : 0,
    custo_por_conversa: mensagens > 0 ? spend / mensagens : 0,
  };
}

/** Soma um conjunto de linhas normalizadas (sem métricas derivadas). */
