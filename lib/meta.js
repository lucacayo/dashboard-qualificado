...
export function withDerived(base) {
  const { spend, impressions, clicks, link_clicks: linkClicks, resultados } = base;
  return {
    ...base,
...
/** Série diária da conta no período. */
export async function fetchDailySeries(accountId, timeRange) {
  const rows = await graphAll(`act_${accountId}/insights`, {
    level: 'account',
    fields: INSIGHT_FIELDS_CONTA,
    time_range: timeRange,
