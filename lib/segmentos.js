...
export function classifyCampaign(campaignName, objective) {
  ...
  if (/\bia\b/.test(n)) destino = 'ia';
    ...
    /* A linha 1498 é atendida pelo time humano. Vale para as campanhas
       [WPP] que só trazem o número, como a "[WPP] [Previ] [WPP-1498]
       INSS Negou" — nas de formulário a marca [Planilha] já resolve. */
    destino = 'humano';
  } else if (tipo === 'wpp' && area === 'civel') {
    /* Regra do negócio, não do nome: em Cível a prospecção pelo
       WhatsApp é feita pela IA, mesmo sem a marca [IA] na campanha. */
    destino = 'ia';
  }
